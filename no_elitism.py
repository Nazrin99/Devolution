...
# Password to crack
TARGET_PASSWORD = "4:qcmA1yd&20p8cyMImCNDx8%"

# Parameters
POPULATION_SIZE = 100  # Reduced for efficiency
MUTATION_RATE = 0.4
GENE_POOL = string.ascii_letters + string.digits + string.punctuation
TOURNAMENT_RATIO = 0.3  # Top 50% of the population for tournament selection
...
def genetic_algorithm(fitness_function):
    ...
    while True:
        ...
        # Fill the population using tournament selection from top 50%
        top_50_percent = sorted_population[:int(POPULATION_SIZE * TOURNAMENT_RATIO)]

        while len(next_generation) < POPULATION_SIZE:
            parent1, parent2 = random.sample(top_50_percent, 2)  # Select parents from top 50%
