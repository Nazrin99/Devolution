...
def genetic_algorithm(fitness_function):
    ...
    while True:
        # Evaluate fitness in parallel
        print(f"Evaluating fitness for generation {generation}...")
        ...
        print(f"Best candidate: {best_candidate}, Fitness: ({best_fitness} / {len(TARGET_PASSWORD)})\n")
        ...
        # Pause after 5 consecutive wrong attempts
        if consecutive_wrong_attempts >= 5:
            print(f"Pausing for 5 seconds due to 5 consecutive wrong attempts...")
            time.sleep(5)
            consecutive_wrong_attempts = 0  # Reset the counter after pausing
...
print(f"Password cracked: {result} in {generations} generations, Time taken: {time_taken:.2f} seconds.")
